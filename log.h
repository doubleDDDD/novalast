...
/* 88 bytes now */
struct nova_inode_complete_entry {
	/* first 40 bytes */
...
#define INODE_COMPLETE_ENTRY(entry)	((struct nova_inode_complete_entry *) entry)
	...
	enum nova_entry_type type)
{
	...
	switch (type) {
	case FILE_WRITE:
		size = sizeof(struct nova_file_write_entry);// 64 B
		break;
	case DIR_LOG:
		size = NOVA_DENTRY_HEADER_LEN; //48 B
		break;
	case SET_ATTR:
	...
	case NOVA_INODE_COMPLETE:
		size = sizeof(struct nova_inode_complete_entry);
		break;
	default:
		break;
	}
...
int nova_inplace_update_write_entry(struct super_block *sb,
	...
	struct nova_log_entry_info *entry_info);
int nova_append_mmap_entry(struct super_block *sb,
