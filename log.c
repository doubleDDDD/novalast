	...
	void *entry, struct nova_log_entry_info *entry_info)
{
	...
	case NOVA_INODE_COMPLETE:
		memcpy_to_pmem_nocache(entry, entry_info->data,
				sizeof(struct nova_inode_complete_entry));
		break;
	default:
		break;
	}
...
static int nova_append_log_entry(struct super_block *sb, struct inode *inode,
	struct nova_inode_info_header *sih, struct nova_log_entry_info *entry_info)
{
	...
	curr_p = nova_get_append_head(sb, sih, tail, size,
						MAIN_LOG, 0, &extended);
	if (curr_p == 0)
		return -ENOSPC;
	...
	struct nova_log_entry_info *entry_info)
{
	...
	pair = &sbi->journal_ptr_pairs[cpu];
	journal_tail = nova_create_logentry_transaction(sb, entry, type, cpu, pair);
	nova_update_log_entry(sb, inode, entry, entry_info);

	PERSISTENT_BARRIER();

	nova_commit_lite_transaction(sb, journal_tail, cpu, pair);
...
/* when init a new inode (only init new inode), allocate a log and append first log entry ---- nova_inode_complete_entry */
int nova_append_inode_complete_entry(struct super_block *sb,
	struct inode *inode, struct nova_inode_update *update,
	struct nova_inode_complete_entry *data)
{
	struct nova_inode_info *si = NOVA_I(inode);
	struct nova_inode_info_header *sih = &si->header;
	struct nova_log_entry_info entry_info;
	...
	NOVA_START_TIMING(append_inode_complete_t, append_time);
	nova_update_entry_csum(data);

	entry_info.type = NOVA_INODE_COMPLETE;
	entry_info.update = update;
	entry_info.data = data;

	ret = nova_append_log_entry(sb, inode, sih, &entry_info);
	if (ret)
		nova_err(sb, "%s failed\n", __func__);

	NOVA_END_TIMING(append_inode_complete_t, append_time);
	return ret;
}

/* Returns new tail after append */
static int nova_append_setattr_entry(struct super_block *sb,
	struct inode *inode, struct iattr *attr, struct nova_inode_update *update,
...
int nova_handle_setattr_operation(struct super_block *sb, struct inode *inode,
	unsigned int ia_valid, struct iattr *attr, u64 epoch_id)
{
	struct nova_inode_info *si = NOVA_I(inode);
	struct nova_inode_info_header *sih = &si->header;
	...
	if (!(ia_valid & ATTR_SIZE) &&
		...
		ret = nova_append_setattr_entry(sb, inode, attr, &update,
		...
		nova_update_inode(sb, inode, &update, 1);
	}
 ...
 * multiple entries.
 */
int nova_append_file_write_entry(struct super_block *sb,
...
int nova_append_mmap_entry(struct super_block *sb, struct inode *inode,
	struct nova_mmap_entry *data, struct nova_inode_update *update,
...
int nova_append_snapshot_info_entry(struct super_block *sb,
	struct nova_inode_info *si, struct snapshot_info *info,
	struct nova_snapshot_info_entry *data, struct nova_inode_update *update)
{
	...
	ret = nova_append_log_entry(sb, NULL, sih, &entry_info);
	if (ret)
		nova_err(sb, "%s failed\n", __func__);
...
int nova_append_dentry(struct super_block *sb, struct inode *dir,
	struct dentry *dentry, u64 ino, unsigned short de_len,
	struct nova_inode_update *update, int link_change, u64 epoch_id)
{
	...
	*/
	...
	ret = nova_append_log_entry(sb, dir, sih, &entry_info);
	if (ret)
		nova_err(sb, "%s failed\n", __func__);
...
static int nova_initialize_inode_log(struct super_block *sb,
	struct nova_inode_info_header *sih, int log_id)
{
	u64 new_block;
	int allocated;
	...
	/* per write lock of AIN */
	write_lock(&pi_ain_loc->rblock_instance);
	pi_ssim = pi_ain_loc->ain_location;
	nova_memunlock_inode_ssim(sb, pi_ssim);
	if (log_id == MAIN_LOG) {
		pi_ssim->log_tail = new_block;
		nova_flush_buffer(&pi_ssim->log_tail, CACHELINE_SIZE, 0);
		pi_ssim->log_head = new_block;
		sih->log_head = sih->log_tail = new_block;
		sih->log_pages = 1;
		nova_flush_buffer(&pi_ssim->log_head, CACHELINE_SIZE, 1);
	} else {
		/*
 ...
 */
static u64 nova_extend_inode_log(struct super_block *sb,
	struct nova_inode_info_header *sih, u64 curr_p)
{
	...
	if (curr_p == 0) {
		ret = nova_initialize_inode_log(sb, sih, MAIN_LOG);
		if (ret)
			return 0;
		/*
		if (metadata_csum) {
...
//	nova_print_inode_log_page(sb, inode);
	...
	*/
	...
	//nova_inode_log_fast_gc(sb, sih, curr_p, new_block,
	//	alter_new_block, allocated, 0);
...
u64 nova_get_append_head(struct super_block *sb, struct nova_inode_info_header *sih,
	u64 tail, size_t size, int log_id, int thorough_gc, int *extended)
{
	u64 curr_p;
	...
	if (curr_p == 0 || (is_last_entry(curr_p, size) &&
		...
		if (thorough_gc == 0) {
			curr_p = nova_extend_inode_log(sb, sih, curr_p);
		} else {
...
int nova_free_inode_log(struct super_block *sb, struct nova_inode_info_header *sih)
{
	...
	/*
		...
		But in SSIM, when the AIN is invalid, we just ignore it and do nothing
	*/
	/*
	if (pi) {
		nova_memunlock_inode(sb, pi);
