...
extern unsigned long *p_global;//added by double_D
extern void *virt_addr_global;//added by double_D

extern int writeop_stat;
extern int metadata_writeop_statistics;
extern int data_writeop_statistics;
...
#define NOVA_INODE_RESERVED_SIZE	128    	/* must be power of two */
#define NOVA_INODE_COMPLETE_SIZE	88
#define NOVA_INODE_SSIM_SIZE 		64
#define AIN_VALID_THRESHOLD 		50
#define MAX_GROUP_PER_CPU			20
#define INODE_LOC_HASH_INIT_SIZE	8192
...
static inline void nova_flush_buffer(void *buf, uint32_t len, bool fence)
{
	...
	if(writeop_stat == 1){
		p_global[((u64)buf-(u64)virt_addr_global) >> PAGE_SHIFT]++;
	}
