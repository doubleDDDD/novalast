...
static int nova_init_root_inode_rebuild(struct super_block *sb,
	struct nova_inode_rebuild *reb, struct nova_inode_reserved *pi_root)
{
	struct nova_inode_reserved fake_pi;
	int rc;

	rc = memcpy_mcsafe(&fake_pi, pi_root, sizeof(struct nova_inode_reserved));
	if (rc)
		return rc;
...
static int nova_rebuild_root_inode_start(struct super_block *sb,
	struct nova_inode_reserved *pi_root, struct nova_inode_info_header *sih,
	struct nova_inode_rebuild *reb)
{
	int ret;

	ret = nova_get_root_head_tail(sb, pi_root, sih);
	if (ret)
		return ret;

	ret = nova_init_root_inode_rebuild(sb, reb, pi_root);
	if (ret)
		return ret;
...
static void nova_update_root_inode_with_rebuild(struct super_block *sb,
	struct nova_inode_rebuild *reb, struct nova_inode_reserved *pi)
{
...
static int nova_rebuild_root_inode_finish(struct super_block *sb,
	struct nova_inode_reserved *pi_root, struct nova_inode_info_header *sih,
	struct nova_inode_rebuild *reb, u64 curr_p)
{
	...
	nova_update_root_inode_with_rebuild(sb, reb, pi_root);
	//nova_update_inode_checksum(pi_root);
	/*
	if (metadata_csum) {
	...
	struct nova_inode_info_header *sih ,struct nova_inode_rebuild *reb)
{
	...
	struct nova_inode_ssim fake_pi_ssim;
	struct nova_inode_complete_entry fake_pi_complete;
	int rc;

	pi_ain_loc = NOVA_HLOC(sih);
	read_lock(&pi_ain_loc->rblock_instance);

	pi_ssim = pi_ain_loc->ain_location;
	rc = memcpy_mcsafe(&fake_pi_ssim, pi_ssim, sizeof(struct nova_inode_ssim));
	if (rc){
		read_unlock(&pi_ain_loc->rblock_instance);
		return rc;
	}
	rc = memcpy_mcsafe(&fake_pi_complete, pi_ssim->complete_inode, sizeof(struct nova_inode_complete_entry));
	if (rc){
		read_unlock(&pi_ain_loc->rblock_instance);
		return rc;
	}
	...
	sih->i_blk_type = fake_pi_complete.i_blk_type;
	sih->log_head = fake_pi_ssim.log_head;
	sih->log_tail = fake_pi_ssim.log_tail;
	sih->alter_log_head = fake_pi_complete.alter_log_head;
	sih->alter_log_tail = fake_pi_complete.alter_log_tail;

	reb->i_flags = le32_to_cpu(fake_pi_ssim.i_flags);
	reb->i_mode = le16_to_cpu(fake_pi_ssim.i_mode);
	reb->i_size = le64_to_cpu(fake_pi_complete.i_size);
	reb->i_uid = le32_to_cpu(fake_pi_complete.i_uid);
	reb->i_gid = le32_to_cpu(fake_pi_complete.i_gid);
	reb->i_atime = le32_to_cpu(fake_pi_complete.i_atime);
	reb->i_ctime = le32_to_cpu(fake_pi_complete.i_ctime);
	reb->i_mtime = le32_to_cpu(fake_pi_complete.i_mtime);
	reb->i_generation = le32_to_cpu(fake_pi_complete.i_generation);
	reb->i_links_count = le16_to_cpu(fake_pi_complete.i_links_count);
	reb->trans_id = 0;
	...
	struct nova_inode_info_header *sih, struct nova_inode_rebuild *reb)
{
	...
	ret = nova_init_inode_rebuild(sb, sih, reb);
	if (ret)
		return ret;
...
static void nova_append_inode_with_rebuild(struct super_block *sb,
	struct nova_inode_rebuild *reb, struct nova_inode_info_header *sih)
{
	struct nova_ain_loc *pi_ain_loc;
	struct nova_inode_ssim *pi_ssim;
	...
	/* step1: update AIN_inplace actually, flags and mode */
	pi_ssim->i_flags = cpu_to_le32(reb->i_flags);
	pi_ssim->i_mode = cpu_to_le16(reb->i_mode);
	...
	/* step2: append new inode_complete_entry to log, update firstly */
	nova_memunlock_inode_entry_complete(sb, pi_complete);
	pi_complete->i_size = cpu_to_le64(reb->i_size);
	pi_complete->i_uid = cpu_to_le32(reb->i_uid);
	pi_complete->i_gid = cpu_to_le32(reb->i_gid);
	pi_complete->i_atime = cpu_to_le32(reb->i_atime);
	pi_complete->i_ctime = cpu_to_le32(reb->i_ctime);
	pi_complete->i_mtime = cpu_to_le32(reb->i_mtime);
	pi_complete->i_generation = cpu_to_le32(reb->i_generation);
	pi_complete->i_links_count = cpu_to_le16(reb->i_links_count);
	nova_flush_buffer(pi_complete, NOVA_INODE_COMPLETE_SIZE, 0);
	...
	u64 curr_p)
{
	...
	nova_append_inode_with_rebuild(sb, reb, sih);
...
static int nova_rebuild_file_inode_tree(struct super_block *sb, struct nova_inode_info_header *sih)
{
	...
	reb = &rebuild;
	ret = nova_rebuild_inode_start(sb, sih, reb);
	if (ret)
		goto out;
...
//	nova_print_nova_log(sb, sih);
	...
	ret = nova_rebuild_inode_finish(sb, sih, reb, curr_p);
...
int nova_rebuild_dir_inode_tree(struct super_block *sb, struct nova_inode_info_header *sih)
{
	struct nova_sb_info *sbi = NOVA_SB(sb);
	...
	pi_root = nova_get_reserved_inode(sb, 1);
	reb = &rebuild;

	if(sih->ino == 1)
		ret = nova_rebuild_root_inode_start(sb, pi_root, sih, reb);
	else
		ret = nova_rebuild_inode_start(sb, sih, reb);
	...
	if(sih->ino == 1)
		ret = nova_rebuild_root_inode_finish(sb, pi_root, sih, reb, curr_p);
	else
		ret = nova_rebuild_inode_finish(sb, sih, reb, curr_p);
...
int nova_rebuild_root_inode(struct super_block *sb, struct nova_inode_info *si)
{
	//struct nova_sb_info *sbi = NOVA_SB(sb);
	struct nova_inode_info_header *sih = &si->header;
	struct nova_inode_reserved *pi_root;
	...
	nova_init_header(sb, sih, __le16_to_cpu(pi_root->i_mode));

	if (pi_root->deleted == 1) {
		nova_dbg("%s: inode %d has been deleted.\n", __func__, 1);
		return -ESTALE;
	}

	/*
	nova_dbgv("%s: inode %llu, addr 0x%llx, valid %d, head 0x%llx, tail 0x%llx\n",
			__func__, 1, pi_root_addr, pi_root->valid,
			pi_root->log_head, pi_root->log_tail);
			*/

	sih->ino = 1;

	nova_rebuild_dir_inode_tree(sb, sih);
	...
	u64 ino, int rebuild_dir)
{
	...
	*/
	read_lock(&pi_ain_loc->rblock_instance);
	pi_ain_loc->ain_location = nova_get_ssim_by_ino(sb, ino);
	pi_ain = pi_ain_loc->ain_location;
	deleted_temp = pi_ain->deleted;
	i_mode_temp = pi_ain->i_mode;
	log_head_temp = pi_ain->log_head;
	read_unlock(&pi_ain_loc->rblock_instance);

	nova_init_header(sb, sih, __le16_to_cpu(i_mode_temp));
	...
	*/
	...
	case S_IFLNK:
		...
		/* Fall through */
	case S_IFREG:
		nova_rebuild_file_inode_tree(sb, sih);
		break;
	case S_IFDIR:
		if (rebuild_dir)
			nova_rebuild_dir_inode_tree(sb, sih);
...
int nova_restore_snapshot_table(struct super_block *sb, int just_init)
{
	...
	struct nova_inode_complete_entry *pi;
