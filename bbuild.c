...
{
	struct nova_sb_info *sbi = NOVA_SB(sb);
	struct nova_get_inode_by_ino *pi = nova_get_inode_by_ino(sb, NOVA_BLOCKNODE_INO);
...
{
	struct nova_sb_info *sbi = NOVA_SB(sb);
	struct nova_get_inode_by_ino *pi = nova_get_inode_by_ino(sb, NOVA_INODELIST_INO);
	struct nova_inode_info_header sih;
	struct nova_range_node_lowhigh *entry;
...
{
	struct nova_inode_reserved *pi = nova_get_inode_by_ino(sb, NOVA_INODELIST_INO);
	struct nova_inode_info_header sih;
	struct nova_sb_info *sbi = NOVA_SB(sb);
	...
	nova_update_tail_reserved_inode(pi, temp_tail);
...
{
	struct nova_inode_reserved *pi = nova_get_inode_by_ino(sb, NOVA_BLOCKNODE_INO);
	struct nova_inode_info_header sih;
	struct nova_sb_info *sbi = NOVA_SB(sb);
	struct free_list *free_list;
...
static int failure_thread_func(void *data)
{
	...
	struct nova_inode_info_header sih;
	struct task_ring *ring;
	struct nova_inode_complete_entry *pi, fake_pi;
...
{
	struct nova_sb_info *sbi = NOVA_SB(sb);
	struct nova_inode_complete_entry *pi =  nova_get_inode_by_ino(sb, NOVA_BLOCKNODE_INO);
	int ret;
