...
// write operation statistic
unsigned long *p_global;//point to the array stored the count of the write operation
void *virt_addr_global;
...
//added by double_D
int writeop_stat;
int metadata_writeop_statistics;
int data_writeop_statistics;
...
//added by double_D
module_param(writeop_stat, int, 0444);
MODULE_PARM_DESC(writeop_stat, "if open the write_op function");

module_param(metadata_writeop_statistics, int, 0444);
MODULE_PARM_DESC(metadata_writeop_statistics, "Statistic the write operation count of metadate of NOVA");

module_param(data_writeop_statistics, int, 0444);
MODULE_PARM_DESC(data_writeop_statistics, "Statistic the write operation count of data of NOVA");
	...
	struct nova_sb_info *sbi)
{
	...
	// added by double_D
	if( writeop_stat == 1 ){
		virt_addr_global = sbi->virt_addr;
		p_global = (unsigned long *)vmalloc((sbi->initsize) >> (PAGE_SHIFT - 3));
		memset((void*)p_global, 0, (sbi->initsize) >> (PAGE_SHIFT - 3));
		nova_info("The value of p_global is %p\n", p_global);//added by double_D
		if( !p_global ){
			printk(KERN_EMERG "Cannot allocate memory space for recording the count of write operaton!\n");
		}
	}
...
static struct nova_inode_reserved *nova_init(struct super_block *sb,
				      unsigned long size)
{
	unsigned long blocksize;
	struct nova_inode_reserved *root_i, *pi;
	...
	nova_update_reserved_inode(sb, &sbi->snapshot_si->vfs_inode, pi, &update, 1);
	*/
	...
	if (nova_init_inode_des_region(sb) < 0){
		nova_err(sb, "nova_init_inode_des_region!\n");
		return ERR_PTR(-EINVAL);
	}
	...
	nova_memunlock_inode_reserved(sb, root_i);
	...
	nova_memlock_inode_reserved(sb, root_i);

	epoch_id = nova_get_epoch_id(sb);
	nova_append_dir_init_entries_root(sb, root_i, epoch_id);
...
static void nova_root_check(struct super_block *sb, struct nova_inode_reserved *root_pi)
{
...
{
	struct nova_inode_reserved *root_pi;
	...
	BUILD_BUG_ON(sizeof(struct nova_inode_reserved) > NOVA_INODE_RESERVED_SIZE);
	BUILD_BUG_ON(sizeof(struct nova_inode_ssim) > NOVA_INODE_SSIM_SIZE);
	//BUILD_BUG_ON(sizeof(struct nova_inode_complete_entry) > NOVA_INODE_COMPLETE_SIZE);
	...
	if (nova_alloc_journal_ptr_pairs_init(sb)) {
		retval = -ENOMEM;
		nova_err(sb, "%s: Failed to allocate journal_ptr_pairs_init!\n",
			__func__);
		goto out;
	}

	if(nova_alloc_curr_ain_location_init(sb)){
		retval = -ENOMEM;
		nova_err(sb, "%s: Failed to allocate AIN current allocation location in DRAM!\n",
			__func__);
		goto out;
	}

	if(nova_alloc_inode_loc_hash(sb)){
		retval = -ENOMEM;
		nova_err(sb, "%s: Failed to allocate hash table in DRAM for the relationship between inode number and the location of AIN!\n",
			__func__);
		goto out;
	}

	if(nova_ain_group_des_init(sb)){
		retval = -ENOMEM;
		nova_err(sb, "%s: Failed to allocate AIN group des in DRAM!\n",
			__func__);
		goto out;
	}
...
setup_sb:
	...
	/* NOVA_ROOT_INO is 1 */
	root_i = nova_iget(sb, NOVA_ROOT_INO);/* root_i is vfs_inode */
	if (IS_ERR(root_i)) {
...
void nova_destroy_inode(struct inode *inode)
{
	nova_dbgv("%s: %lu\n", __func__, inode->i_ino);
...
// the output operation of nfs
static const struct export_operations nova_export_ops = {
	.fh_to_dentry	= NULL,//nova_fh_to_dentry,
	.fh_to_parent	= NULL,//nova_fh_to_parent,
	.get_parent	= NULL,//nova_get_parent,
};
