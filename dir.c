...
struct nova_dentry *nova_find_dentry(struct super_block *sb,
	struct inode *inode, const char *name, unsigned long name_len)
{
...
int nova_append_dir_init_entries(struct super_block *sb,
	struct inode *inode, u64 parent_ino, u64 epoch_id)
{
	struct nova_inode_info_header sih;
	...
	/* it is a new AIN, so the update is ok now, no move will happen */
	write_lock(&pi_ain_loc->rblock_instance);
	pi_ssim = pi_ain_loc->ain_location;
	nova_memunlock_inode_ssim(sb, pi_ssim);

	pi_ssim->log_tail = pi_ssim->log_head = new_block;

	pi_ssim->log_tail = new_block + length;
	nova_flush_buffer(&pi_ssim->log_tail, CACHELINE_SIZE, 1);
...
/* init root ino dentry */
int nova_append_dir_init_entries_root(struct super_block *sb,
	struct nova_inode_reserved *pi, u64 epoch_id)
{
	struct nova_inode_info_header sih;
	//struct nova_inode_complete_entry *alter_pi;
	//u64 alter_pi_addr = 0;
	int allocated;
	//int ret;
	u64 new_block;
	...
	de_entry = (struct nova_dentry *)nova_get_block(sb, new_block);
	length = nova_init_dentry(sb, de_entry, NOVA_ROOT_INO, NOVA_ROOT_INO, epoch_id);
	nova_update_tail_reserved_inode(pi, new_block + length);
	...
	nova_memunlock_inode_complete(sb, pi);
	pi->alter_log_tail = pi->alter_log_head = new_block;
	de_entry = (struct nova_dentry *)nova_get_block(sb, new_block);
	length = nova_init_dentry(sb, de_entry, self_ino, parent_ino, epoch_id);
	nova_update_alter_tail_reserved_inode(pi, new_block + length);


	// when i finish the up half, and then process the alter part
	nova_update_alter_pages(sb, pi, pi->log_head,
						pi->alter_log_head);
	nova_update_inode_checksum(pi);
	nova_flush_buffer(pi, sizeof(struct nova_inode), 0);
	...
	struct nova_inode_update *update, u64 epoch_id)
{
	...
	ret = nova_append_dentry(sb, dir, dentry, ino,
				loglen, update, inc_link, epoch_id);
	...
	struct nova_inode_update *update, u64 epoch_id)
{
	...
	ret = nova_append_dentry(sb, dir, dentry, 0, loglen,
				update, dec_link, epoch_id);
...
static int nova_readdir_slow(struct file *file, struct dir_context *ctx)
{
	...
	ret = 0;//nova_readdir_slow_rbtree(file, ctx);
...
static int nova_readdir_fast(struct file *file, struct dir_context *ctx)
{
	...
	struct super_block *sb = inode->i_sb;
	struct nova_inode_info *si = NOVA_I(inode);
	struct nova_inode_info *child_si = NULL;
	struct nova_inode_info_header *sih = &si->header;
	struct nova_dentry *entry = NULL;
	struct nova_dentry *entryc, entry_copy;
	...
	while (curr_p != sih->log_tail) {
		...
		// ergodic the log, only process dentry
		...
		if (entryc->ino > 0 && entryc->invalid == 0
			...
			if (prev_entry)
			{
				if(prev_ino == 1){
					...
					if (!dir_emit(ctx, prev_entryc->name, prev_entryc->name_len, ino,
						IF2DT(le16_to_cpu(pi_root->i_mode)))) {
						nova_dbgv("Here: pos %llu\n", ctx->pos);
						return 0;
					}
				}
				...
				{
					child_inode = nova_iget_only_init_lock(sb, prev_ino);
					child_si = NOVA_I(child_inode);
					pi_ain_loc =  &child_si->ain_loc_and_lock;
					read_lock(&pi_ain_loc->rblock_instance);
					pi_ssim = nova_get_ssim_by_ino(sb, prev_ino);
					prev_imode = pi_ssim->i_mode;
					read_unlock(&pi_ain_loc->rblock_instance);

					if (child_inode->i_state & I_NEW){
						clear_inode(child_inode);
						nova_destroy_inode(child_inode);
					}

					if(!dir_emit(ctx, prev_entryc->name, prev_entryc->name_len, ino, IF2DT(le16_to_cpu(prev_imode))))
					{
						nova_dbgv("Here: pos %llu\n", ctx->pos);
						return 0;
					}
	...
	if (prev_entry)
	{
		if(prev_ino == 1){
			pi_root = nova_get_reserved_inode(sb, 1);
			if (!dir_emit(ctx, prev_entryc->name, prev_entryc->name_len, ino, IF2DT(le16_to_cpu(pi_root->i_mode)))) {
				nova_dbgv("Here: pos %llu\n", ctx->pos);
				return 0;
			}
		}
