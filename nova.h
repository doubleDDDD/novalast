	...
	unsigned int size)
{
	...
	if(writeop_stat == 1){
		for(blocknr = ((u64)dst-(u64)virt_addr_global) >> PAGE_SHIFT;
			blocknr != (((u64)dst + size - (u64)virt_addr_global) >> PAGE_SHIFT) + 1; ++blocknr){
			(*(p_global+blocknr))++;
		}
	}
...
static inline void memset_nt(void *dest, uint32_t dword, size_t length)
{
	...
	if(writeop_stat == 1){
		for(blocknr = ((u64)dest-(u64)virt_addr_global) >> PAGE_SHIFT;
			blocknr != (((u64)dest + length - (u64)virt_addr_global) >> PAGE_SHIFT) + 1; ++blocknr){
			(*(p_global+blocknr))++;
		}
	}
...
#include "inode.h"
static inline int nova_get_root_head_tail(struct super_block *sb,
...
static inline void nova_set_next_page_address_and_nr(struct super_block *sb,
	struct nova_journal_page *prev_page, u64 next_page, unsigned long total_pages, int fence, unsigned long first_blocknr)
{
	prev_page->page_tail.next_page = next_page;
	prev_page->page_tail.blocknr_total = total_pages;
	if( total_pages == JOURNAL_BLOCK_LIST_LEN )
		prev_page->page_tail.first_blocknr = first_blocknr;
	nova_flush_buffer(&prev_page->page_tail,
...
/* gc.c */
int nova_inode_log_fast_gc(struct super_block *sb, struct nova_inode_info_header *sih,
