	...
	size_t len, loff_t *ppos)
{
	...
	pos = *ppos;//0
	index = pos >> PAGE_SHIFT;//0
	offset = pos & ~PAGE_MASK;//0
	...
	entryc = (metadata_csum == 0) ? entry : &entry_copy;
	end_index = (isize - 1) >> PAGE_SHIFT;//1


	printk("In this read, the inode is %lu", sih->ino);
	...
	do {
		...
		printk("curr entry info: entry pgoff %llu, num %u, blocknr %llu\n, size is %llu, reassigned is %d, invalid_pages is %u",
			entry->pgoff,
			entry->num_pages,
		...
		if (index < entryc->pgoff ||
			...
			nova_err(sb, "%s ERROR: %lu, entry pgoff %llu, num %u, blocknr %llu\n, inode number is %lu",
				...
				entry->num_pages, entry->block >> PAGE_SHIFT, sih->ino);
			return -EINVAL;
		}
		*/
...
skip_verify:
		...
		if (!zero)
			// the left represents how many bytes can not be copied
	...
	const char __user *buf,	size_t len, loff_t *ppos)
{
	...
	//it is current position of read or write
	pos = *ppos;//0
	...
	*/
	...
	offset = pos & (sb->s_blocksize - 1);//offset in one block, 0
	num_blocks = ((count + offset - 1) >> sb->s_blocksize_bits) + 1;//the block numbers, if the len is 4096+2048, the block is 2
	total_blocks = num_blocks;//2
	//>> 12 bits, actually the start_blk is the num of block
	start_blk = pos >> sb->s_blocksize_bits;//0
	...
	update.alter_tail = sih->alter_log_tail;
	while (num_blocks > 0) {
		offset = pos & (nova_inode_blk_size(sih) - 1);//0
		start_blk = pos >> sb->s_blocksize_bits;//0
		...
		step++;
		bytes = sb->s_blocksize * allocated - offset;//8192
		if (bytes > count)
			bytes = count;
		//the bytes is real bytes that the process will write to
		...
		if (pos + copied > inode->i_size)
			file_size = cpu_to_le64(pos + copied);//6144
		else
		...
		ret = nova_append_file_write_entry(sb, inode,
					&entry_data, &update);
