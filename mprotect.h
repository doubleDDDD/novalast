...
static inline void nova_memunlock_inode_reserved(struct super_block *sb,
					 struct nova_inode_reserved *pi)
{
	if (nova_range_check(sb, pi, NOVA_INODE_RESERVED_SIZE))
		return;

	if (nova_is_protected(sb))
		__nova_memunlock_range(pi, NOVA_INODE_RESERVED_SIZE);
}

static inline void nova_memlock_inode_reserved(struct super_block *sb,
				       struct nova_inode_reserved *pi)
{
	/* nova_sync_inode(pi); */
	if (nova_is_protected(sb))
		__nova_memlock_range(pi, NOVA_INODE_RESERVED_SIZE);
}

static inline void nova_memunlock_inode_entry_complete(struct super_block *sb,
					 struct nova_inode_complete_entry *pi)
{
	if (nova_range_check(sb, pi, NOVA_INODE_COMPLETE_SIZE))
		return;

	if (nova_is_protected(sb))
		__nova_memunlock_range(pi, NOVA_INODE_COMPLETE_SIZE);
}

static inline void nova_memlock_inode_entry_complete(struct super_block *sb,
				       struct nova_inode_complete_entry *pi)
{
	/* nova_sync_inode(pi); */
	if (nova_is_protected(sb))
		__nova_memlock_range(pi, NOVA_INODE_COMPLETE_SIZE);
}

static inline void nova_memunlock_inode_ssim(struct super_block *sb,
					 struct nova_inode_ssim *pi)
{
	if (nova_range_check(sb, pi, NOVA_INODE_SSIM_SIZE))
		return;

	if (nova_is_protected(sb))
		__nova_memunlock_range(pi, NOVA_INODE_SSIM_SIZE);
}

static inline void nova_memlock_inode_ssim(struct super_block *sb,
				       struct nova_inode_ssim *pi)
{
	/* nova_sync_inode(pi); */
	if (nova_is_protected(sb))
		__nova_memlock_range(pi, NOVA_INODE_SSIM_SIZE);
}
