				 ...
				 struct nova_dentry **res_entry)
{
	...
	direntry = nova_find_dentry(sb, dir, entry->name, entry->len);
	if (direntry == NULL)
		return 0;
				   ...
				   unsigned int flags)
{
	...
	ino = nova_inode_by_name(dir, &dentry->d_name, &de); // inode never be root
	...
	struct nova_inode_update *update)
{
	...
	journal_tail = nova_create_inode_transaction(sb, inode, dir, cpu, 1, 0, pair);

	nova_update_inode(sb, dir, update, 0);// pi_dir

	/* update the valid_ino of a new AIN atomically */
	write_lock(&pi_ain_loc->rblock_instance);
	pi_ssim = pi_ain_loc->ain_location;
	pi_ssim->valid_ino = 1;// pi
 ...
 * with d_instantiate().
 */
static int nova_create(struct inode *dir, struct dentry *dentry,
			umode_t mode, bool excl)
{
	...
	/* only an inode number */
	ino = nova_new_nova_inode(sb);
	if (ino == 0)
		goto out_err;
	...
	inode = nova_new_vfs_inode(TYPE_CREATE, dir, ino, mode, 0,
	...
	nova_lite_transaction_for_new_inode(sb, inode, dir, &update);
...
static void nova_lite_transaction_for_time_and_link(struct super_block *sb,
	struct inode *inode, struct inode *dir, struct nova_inode_update *update,
	struct nova_inode_update *update_dir, int invalidate, u64 epoch_id)
{
	...
	if (invalidate) {
		...
		pi_ssim = pi_ain_loc->ain_location;
		pi_ssim->valid_ino = 0;
		(pi_ssim->complete_inode)->delete_epoch_id = epoch_id;
		write_unlock(&pi_ain_loc->rblock_instance);
	}

	nova_update_inode(sb, inode, update, 0);
	nova_update_inode(sb, dir, update_dir, 0);
...
static int nova_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	update.tail = 0;
	//update.alter_tail = 0;
	retval = nova_append_link_change_entry(sb, inode, &update,
						&old_linkc, epoch_id);
	if (retval)
		goto out;

	nova_lite_transaction_for_time_and_link(sb, inode, dir,
...
static int nova_mkdir(struct inode *dir, struct dentry *dentry, umode_t mode)
{
	...
	inode = nova_new_vfs_inode(TYPE_MKDIR, dir, ino,
	...
	err = nova_append_dir_init_entries(sb, inode, dir->i_ino,
					epoch_id);
	if (err < 0)
		goto out_err;
...
static int nova_rmdir(struct inode *dir, struct dentry *dentry)
{
	...
	err = nova_append_link_change_entry(sb, inode, &update,
						&old_linkc, epoch_id);
	if (err)
		goto end_rmdir;
