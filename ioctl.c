...
long nova_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	struct nova_inode_ssim *pi_ssim = pi_ain_loc->ain_location;
	...
	case FS_IOC_SETFLAGS: {
		...
		oldflags = le32_to_cpu(pi_ssim->i_flags);
		...
		nova_set_inode_flags(inode, pi_complete, flags);
		sih->i_flags = flags;

		update.tail = 0;
		update.alter_tail = 0;
		ret = nova_append_link_change_entry(sb, inode,
